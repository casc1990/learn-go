...
func main()  {
	...
	//指针数组：它是一个数组，但是每个元素是指针类型； 指向数组的指针：它是指针类型，指向了底层数组
	var p *[10]int = &f  /* 指向数组的指针  定义指针：*<type>；&<变量>：取变量地址 */
	fmt.Printf("指针指向底层数组的值:%d\n",*p) //[1 0 0 0 5 0 0 0 0 10]
	x,y := 11,22
	/* p1 := [2]*int{&x,&y} */
	var p1 [2]*int  /* 指针数组；这个指针数组有2个元素  */
	p1[0] = &x
	p1[1] = &y  /* 等同于：p1 := [2]*int{&x,&y} */
	fmt.Printf("指针数组里第一个元素的值是: %d\n",*p1[0]) //输出: 指针数组的值是: 11;
	...
	fmt.Println(k) /* 输出：[0 1 2 3 5 6 9] */
